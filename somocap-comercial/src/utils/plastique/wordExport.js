...
export const generateWordReport = async (data, columnNames, valueColumns, images, isRedField) => {
  // For plastique, we'll create a simple report with both tables
  const documentContent = [
    new Paragraph({
      children: [new TextRun({ text: "Rapport Plastique", size: 32, bold: true })],
  ...
  // Add each table
  TABLES.forEach(({ key, title, headers }) => {
    ...
    // Create data rows
    const dataRows = headers.map((header, index) => {
      const cells = [
        new TableCell({
          children: [new Paragraph({
      ...
      for (let col = 0; col < valueColumns; col++) {
        const cellValue = data[col]?.[key]?.[header] || '';
    ...
    documentContent.push(table);
    documentContent.push(new Paragraph({ text: "", spacing: { after: 300 } })); // Spacer
  });
